...
def isAlphabet(ch) -> bool:
    ...
    LOWER_A_LETTER = 97
    LOWER_Z_LETTER = 122
    return (ord(ch) >= UPPER_A_LETTER and ord(ch) <= UPPER_Z_LETTER) or (
        ord(ch) >= LOWER_A_LETTER and ord(ch) <= LOWER_Z_LETTER
    )
...
def get_optimized_font_size(lyrics: list[str]) -> int:
    max_font_size = 40
    min_font_size = 10
    max_unit_length = max([get_unit_length(lyric) for lyric in lyrics])

    font_size = max_font_size - max_unit_length * 0.5
